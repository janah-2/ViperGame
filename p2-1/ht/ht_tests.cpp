...
TEST(RemoveTest, InsertandRemove)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	// remove data at key 1
    void* data = removeItem(ht, 1);

	// correct data m[1] should be returned after removing data (data removed)
    EXPECT_EQ(m[1], data);
...
TEST(RemoveTest, InsertandDelete)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	// delete item at key 0
    deleteItem(ht, 0);

	// since item was deleted, data should return NULL
    EXPECT_EQ(NULL, getItem(ht,0));

	// remove item at key 1
    void* data = removeItem(ht, 1);
...
TEST(RemoveTest, RemoveAndDelete)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
